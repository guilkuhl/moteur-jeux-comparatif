...
def api_convert(payload: ConvertRequest) -> JobCreatedResponse:
    ...
    # Les algos sont CPU-bound (NumPy/Pillow relâchent le GIL) : un thread
    # suffit. L'event loop asyncio reste réactif pour servir le flux SSE.
    threading.Thread(
        target=run_job,
...
@router.get("/algos")
def api_algos() -> dict:
    """Catalogue des algos disponibles pour le client, construit depuis ALGO_MODULES."""
    result: dict = {}
